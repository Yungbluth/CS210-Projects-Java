...
public class SpaceInvaders extends Application {
	...
	public void start(Stage stage) {
		...
		canvas.setOnMouseMoved((event)->{axis = event.getX();
			gc.clearRect(0, WINDOW_SIZE - SHIP_SIZE, WINDOW_SIZE, SHIP_SIZE);
			gc.drawImage(ship, axis - (SHIP_SIZE / 2), WINDOW_SIZE - SHIP_SIZE, SHIP_SIZE, SHIP_SIZE);});
		canvas.setOnMouseClicked((event)->{gc.setFill(Color.BLACK);
			Timeline t = new Timeline(new KeyFrame(Duration.millis(MILLISECONDS), new Animation(event.getX(), gc)));
